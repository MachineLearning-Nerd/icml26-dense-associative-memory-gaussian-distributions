"""Frozen copy of the Claim 1 verifier used by the recorded run."""
...
def verify(output_dir):
    ...
    distances = np.array(
        [core.wasserstein_squared(pattern, query) for pattern in patterns]
    )
    ...
    centroid = (
        ...
        np.mean(
            np.stack([covariance for _, covariance in patterns]), axis=0
        ),
    )
